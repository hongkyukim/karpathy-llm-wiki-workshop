...
def main() -> None:
    ...
    for sub in ["raw/github", "raw/assets", "entities", "concepts", "comparisons", "queries"]:
        (vault / sub).mkdir(parents=True, exist_ok=True)
        write_if_missing(vault / sub / ".gitkeep", "")
    today = date.today().isoformat()
    write_if_missing(vault / "SCHEMA.md", SCHEMA)
    write_if_missing(vault / "index.md", f"""# Wiki Index
...
## Queries
""")
    write_if_missing(vault / "log.md", f"""# Wiki Log

> Chronological record of wiki actions. Append-only.
> Format: `## [YYYY-MM-DD] action | subject`

## [{today}] create | Wiki initialized
- Created Obsidian-compatible LLM Wiki structure.
""")
    print(f"Initialized vault at {vault.resolve()}")
